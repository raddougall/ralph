...
- `JARVIS_CODEX_CAPABILITY_PREFLIGHT_INCLUDE_CLICKUP` (default: `0`; include ClickUp API host in nested capability probe only when needed)
- `JARVIS_CODEX_CAPABILITY_PREFLIGHT_REQUIRE_NESTED_GIT_WRITE` (default: auto; `0` in runner commit mode, `1` in agent commit mode)
...
- `JARVIS_CLICKUP_DISABLE_ON_NESTED_DNS_FAILURE` (default: `0`; when `1`, disable ClickUp actions for run if nested DNS cannot resolve ClickUp)
