...
- `JARVIS_CLICKUP_DIRECTIVES_SYNC_ON_START` (default: `0`, syncs a human-readable Jarvis directives doc in ClickUp before iterations when wrapper is available)
- `JARVIS_CLICKUP_DIRECTIVES_SYNC_STRICT` (default: `0`, set `1` to fail the run if directives sync fails)
- `JARVIS_CLICKUP_DIRECTIVES_SYNC_BRANCH_POLICY` (default: `main_only`, sync directives only on `JARVIS_MAIN_BRANCH`)
- `JARVIS_CLICKUP_AUTO_DEPLOY_ON_MAIN` (default: `0`; when disabled, main completion defaults to `done` and non-main remains `testing`)
