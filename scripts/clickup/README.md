...
For branch-based workflows, keep `JARVIS_CLICKUP_AUTO_DEPLOY_ON_MAIN=0` so `passes=true` maps to `done` on main and `testing` on non-main by default; enable main-only deploy transitions with `JARVIS_CLICKUP_AUTO_DEPLOY_ON_MAIN=1` when desired.
