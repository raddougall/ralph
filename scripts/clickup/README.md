...
This folder contains shared helper scripts to sync `prd.json` stories into a ClickUp list.

Recommended usage: run the project-local wrappers created by `scripts/install-project-launcher.sh` (for example `./scripts/clickup/sync_prd_to_clickup.sh` from your project root).
